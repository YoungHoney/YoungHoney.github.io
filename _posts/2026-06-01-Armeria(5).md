...
ikhoon 리뷰어가 이 구조를 지적하며 개선을 요청했다.

_ikhoon 리뷰어의 코드리뷰 댓글은 별도 첨부 예정_

제안된 구조는 명확했다. 모든 model 정의를 최상위 `$defs.models`로 올리고, 각 메서드는 `$defs.methods`에 모아서 참조로만 연결하자는 것이었다.
...
PR Draft를 올린 것이 2025년 8월이었고, ikhoon에게 merge를 받은 것이 2026년 2월이었다. 반년 가까이 걸린 여정이었지만, 오픈소스 프로젝트에 실질적인 기능을 기여할 수 있어서 보람찼다.
